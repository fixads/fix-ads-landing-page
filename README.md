...
The multilingual design was approved by the project owner and published to `https://www.fixads.xyz` on 2026-07-16 after protected Netlify drafts passed verification. Later CRM and recovery releases are recorded below. The current production deploy is locked release `6aa4148ac5217ae21cb8549c`, published on 2026-09-11 at `14:49:50.729Z`. This owner-introduction removal was cloned from exact locked September 11 baseline `6aa3ffe6d48b541dee4302a5`, retaining the earlier multilingual market refinement, Hebrew-language refinement and protected August 30 Transparency/BluYacht state. The latest change removes the personal owner-led/Berlin/market-experience block and its personal LinkedIn CTA from all three homepages; no replacement block is added. Earlier localized offers, work examples, customer-care explanation, FAQs, metadata, conceptual imagery and the interactive integration example remain. It retains the existing phone-first layout, localized service/legal pages, all ten Function digests and overrides, three schedules, both Forms, legacy files, and the verified 29-rule route set. Netlify automatic Git builds are paused (`build_settings.stop_builds = true`) because the connected `main` branch does not build this protected multilingual production package. Future planned production work must use the protected draft process and explicitly unlock the current deploy only for a verified cutover.
...
- Exact current homepage models match `content.js` in all three locales. Source and matching README were backed up in GitHub commit `58b28de33d18fbb83316bc11fe345db600d3b201` before publication. Reconfirmed the baseline and published the exact verified draft at `2026-09-11T14:49:50.729Z`, then locked it; automatic Git builds remain paused.
- Post-publication verification confirmed all three homepages, three service pages, four legal pages, robots/sitemap/llms files, `/transparency` and `/transparency/dashboard` return 200. HTTP/HTTPS and www/non-www variants resolve to canonical HTTPS on `www.fixads.xyz`. The removed introduction is absent from every localized homepage HTML document, Impressum is German/LTR, and all five protected Transparency hashes remain exact. Root country selection and direct-path preservation passed again on production.
- Live English mobile Lighthouse: 98 Performance, 100 Accessibility, 100 Best Practices, 100 SEO; LCP about 1.4 seconds, CLS 0, zero non-composited animation elements. Report: `output/lighthouse-owner-removal-live-20260911.json`. This is a single lab measurement, not a guarantee for every phone/network or evidence of lead growth.
- Live phone smoke tests passed in English, German and Hebrew at 390px, with reduced motion, menu-to-contact navigation, visible footer language links and no JavaScript page errors or horizontal overflow. Hebrew landscape navigation also passed at 844×390. All three localized forms reached their success state and reset correctly with intercepted/mock POST responses; no test leads or emails were sent, and this does not re-verify real email delivery. Screenshot: `output/playwright/owner-removal-live-he-mobile-20260911.png`.
