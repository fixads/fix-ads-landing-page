...
The multilingual design was approved by the project owner and published to `https://www.fixads.xyz` on 2026-07-16 after protected Netlify drafts passed verification. Later CRM and recovery releases are recorded below. The current production deploy is locked release `6a85b9671986ca901f3b9b8f`, published on 2026-08-19 at `14:27:37.726Z`. It restores the approved multilingual marketing, localized service, and legal pages after manual CLI production deploy `6a85b805f1d52abce2690e12` replaced them with a separate one-page homepage and removed the locale routes. The recovery preserves the exact protected Transparency files from that current production baseline, all ten Functions, all three schedules, both forms, the current legacy files, and the verified 29-rule route set. Netlify automatic Git builds are paused (`build_settings.stop_builds = true`) because the connected `main` branch does not build this protected multilingual production package. Future planned production work must use the protected draft process and explicitly unlock the current deploy only for a verified cutover.
